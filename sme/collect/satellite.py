...
from sme.core.grid import (extract_windows, lonlat_to_rowcol, satellite_zenith,
...
def collect_satellite(start: str, end: str, channels: list[str],
    ...
    tag = "sat_final" if set(channels) == set(kma_client.DEFAULT_CHANNELS) and half == 4 \
    ...
    for di, day in enumerate(days, 1):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futs = {
                ex.submit(kma_client.fetch_gk2a, ch,
...
def collect_asos(start: str, end: str) -> Path:
    df = kma_client.fetch_asos_range(start, end)
...
def main() -> None:
    ...
    ap.add_argument("--channels", default=",".join(kma_client.DEFAULT_CHANNELS))
