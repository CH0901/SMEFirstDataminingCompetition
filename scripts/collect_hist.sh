...
for Y in 2021 2022; do
  [ -f data/asos14_${Y}.parquet ] || $PY -m sme.collect.labels ${Y}0601 ${Y}0930 data/asos14_${Y}.parquet 2>&1 | grep -E "^\[labels\]"
done
# 위성 — 채점 시기와 같은 8~9월만
for Y in 2022 2021 2020 2019; do
  echo "--- 위성 ${Y}년 8~9월 ---"
  $PY -m sme.collect.satellite sat --start ${Y}0801 --end ${Y}0930 2>&1 | grep -E "^\[sat\]" | tail -1
