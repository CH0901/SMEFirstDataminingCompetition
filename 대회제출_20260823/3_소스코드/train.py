...
def main() -> None:
    import kma
    channels = list(kma.DEFAULT_CHANNELS)
    df = eval14.assemble(channels)
    # 채널 파생 + 분리대기창 보정항(SWD_*) 을 모두 포함시킨다.
    feats = eval14.BASE + [c for c in df.columns
    ...
    for target in ("TA14", "HM14"):
        ...
        ds = lgb.Dataset(sub[tg], resid, categorical_feature=eval14.CAT)
        models[target] = [
    ...
    from geo import lonlat_to_rowcol, satellite_zenith
