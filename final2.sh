...
$PY collect_labels.py 20200601 20200930 data/asos14_2020.parquet 2>&1 | grep -E "^\[labels\]"
echo "\n═══ 7년치 재학습 ═══"
$PY train.py 2>&1 | grep -vE "NotOpenSSL|warnings.warn|RuntimeWarning|ret = a|return X @|^  warn"
echo "\n═══ 홀드아웃 종단검증 ═══"
$PY holdout_test.py 2>&1 | grep -vE "NotOpenSSL|warnings.warn|RuntimeWarning|ret = a|return X @|^  warn"
cp model_holdout.pkl /tmp/kgl/input/model/model.pkl
rm -f /tmp/kgl/working/submission.csv
$PY -u rehearsal.py "$KMA_API_KEY2" 20250824 20250830 2>&1 | grep -E "최종 성공|예측 완료"
...
rm -f /tmp/kgl/working/submission.csv
$PY -u rehearsal.py "$KMA_API_KEY" 20260624 20260630 2>&1 | grep -E "최종 성공|예측 완료"
cp /tmp/kgl/working/submission.csv ./submission.csv
