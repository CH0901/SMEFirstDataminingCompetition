...
echo "═══ 최종 학습 ═══"
$PY train.py 2>&1 | grep -vE "NotOpenSSL|warnings.warn|RuntimeWarning|ret = a|return X @|^  warn|Performance|out\[" | head -6
echo "\n═══ 노트북 갱신 ═══"
$PY - <<'EOF'
import json
...
cp /tmp/kgl/working/submission.csv ./submission.csv 2>/dev/null && echo "  submission.csv 갱신"
echo "\n═══ 완료 $(date +%H:%M) ═══"
